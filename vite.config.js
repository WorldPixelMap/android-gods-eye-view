...
function tomtomProxy() {
  ...
  async function fetchUpstream(z, x, y, apiKey) {
    const key = apiKey || process.env.TOMTOM_API_KEY;
    const url = 'https://api.tomtom.com/traffic/map/4/tile/flow/relative/'
      + `${z}/${x}/${y}.pbf?key=${encodeURIComponent(key)}`;
    ...
    configureServer(server) {
      server.middlewares.use('/api/tomtom', async (req, res) => {
        ...
        try {
          ...
          const clientKey = parsedUrl.searchParams.get('key') || req.headers['x-tomtom-key'];
          const tomtomKey = String(clientKey || process.env.TOMTOM_API_KEY || '').trim();

          if (urlPath === '/status') {
            const hasKey = Boolean(tomtomKey);
          ...
          if (!inflight.has(key)) {
            inflight.set(key, fetchUpstream(z, x, y, tomtomKey)
              .then(async (buf) => {
    ...
    configureServer(server) {
      server.middlewares.use('/api/firms', async (req, res) => {
        ...
        try {
          ...
          const queryKey = parsedUrl.searchParams.get('key') || req.headers['x-firms-map-key'];
          const key = String(queryKey || mapKey()).trim();
          await readDiskOnce();
...
function aisLiveProxy() {
  function install(middlewares) {
    middlewares.use('/api/ais-live', async (req, res) => {
      try {
        const incoming = new URL(req.url || '', 'http://localhost');
        const clientKey = String(incoming.searchParams.get('key') || '').trim();
        if (clientKey && !process.env.AISSTREAM_API_KEY) {
          process.env.AISSTREAM_API_KEY = clientKey;
        }
        ...
        res.statusCode = (process.env.AISSTREAM_API_KEY || clientKey) ? 200 : 503;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store');
        res.end(JSON.stringify({
          rows,
